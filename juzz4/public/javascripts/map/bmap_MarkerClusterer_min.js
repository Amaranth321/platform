...
//            thatMap.setViewport(thatBounds);
        	...
        	//start
        	//Special handle to list Marker Clusterer in list
            if(baiduMapManager.infoWindowListOpen == false){
            	baiduMapManager.infoWindowListOpen = true;            	
            	var context 	= "";
            	var data    	= [];
            	var date 		= new Date();
            	var randomid 	= date.getHours()+""+date.getMinutes()+""+date.getSeconds()+""+date.getMilliseconds();
            	var searchbar 	= "<div >"+
            					  "<input id='"+randomid+"' style='width: 300px' />"+
            					  "</div>";   
            	
            	//initialize marker list 
            	for (var i = 0; i<thatMarkers.length ; i++) {
            		for(var g = 0; g<baiduMapManager.infoMarkerList.length ; g++){
            			if(thatMarkers[i].getTitle() == baiduMapManager.infoMarkerList[g].getTitle()){
            				context += "<div class='infocontent'>"+baiduMapManager.infoContentList[g]+"</div>";
            				break;
            			}
            		}
            		data.push(thatMarkers[i].getTitle());
                }
            	
            	//create new pop up window
        		var kendoWindow = $("<div />").kendoWindow({
                    visible: false,
                    title: localizeResource("marker-list"),
                    resizable: false,
                    modal: true,
                    maxHeight: 400,
                    width: 350,
                    close: function(){baiduMapManager.infoWindowListOpen = false;}
                });
                kendoWindow.data("kendoWindow").content(searchbar+context).center().open();
                
                //initialize search bar 
            	var multi = $("#"+randomid).kendoMultiSelect({
            		height: 200,
                    dataSource: data,
                    placeholder: localizeResource("filter-markers"),
                    change: function(e){
                    	var subcontext = "";
                    	$(".infocontent").remove();
                    	if(multi.value().length==0)
                    		kendoWindow.contents().append(context);                    		
                		for (var i = 0; i<multi.value().length ; i++) {
                    		for(var g = 0; g<baiduMapManager.infoMarkerList.length ; g++){
                    			if(multi.value()[i] == baiduMapManager.infoMarkerList[g].getTitle()){
                    				subcontext += "<div class='infocontent'>"+baiduMapManager.infoContentList[g]+"</div>";
                    				break;
                    			}
                    		}
                        }
                		kendoWindow.contents().append(subcontext);
                    }
