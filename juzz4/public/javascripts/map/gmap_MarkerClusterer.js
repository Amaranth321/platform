...
ClusterIcon.prototype.onAdd = function() {
    ...
    google.maps.event.addDomListener(this.div_, "click", function(e) {
        ...
        if (!f) {
        	...
        	for (var i = 0; i<d.cluster_.getMarkers().length ; i++) {
        		for(var g = 0; g<mapManager.infoMarkerList.length ; g++){
        			if(d.cluster_.getMarkers()[i].getTitle() == mapManager.infoMarkerList[g].getTitle()){
        				context += "<div class='infocontent'>"+mapManager.infoContentList[g]+"</div>";
        				break;
        			}
        		}
        		data.push(d.cluster_.getMarkers()[i].getTitle());
            }
        	...
        	var multi = $("#"+randomid).kendoMultiSelect({
                ...
                change: function(e){
            		...
            		for (var i = 0; i<multi.value().length ; i++) {
                		for(var g = 0; g<mapManager.infoMarkerList.length ; g++){
                			if(multi.value()[i] == mapManager.infoMarkerList[g].getTitle()){
                				subcontext += "<div class='infocontent'>"+mapManager.infoContentList[g]+"</div>";
                				break;
                			}
                		}
                    }
